...
st.write("Then, we extracted the data on each company site using the links extracted previously. In the end we had 235503 rows of reviews. For this we had to set a checkpoint in order to not lose data in case the program crashes. So, for each tenth iteration, we save the extracted data and if we re-execute the program by indicating the right iteration, the program continues. This process took time, because we had to set up a sleep(2); otherwise the website would strike us for requesting too much and too fast. This made the extraction take a lot of time.")
